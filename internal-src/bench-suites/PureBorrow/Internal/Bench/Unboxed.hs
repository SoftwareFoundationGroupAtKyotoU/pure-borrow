...
import Control.Concurrent.DivideConquer.Linear qualified as OwningDC
import Control.Concurrent.DivideConquer.Linear.Unrestricted qualified as UnrestrictedDC
...
directFixedUnboxedKernel input =
  unsafePerformIO do
    vector <- U.thaw input
    directWorker (UM.length vector) 0 vector
    U.unsafeFreeze vector

directWorker :: Int -> Int -> UM.IOVector Int -> IO ()
...
directGrowableUnboxedNoGrowthKernel input =
  unsafePerformIO do
    vector <- U.thaw input
    header <- newIORef (U.length input, vector)
    (_, content) <- readIORef header
    directWorker (UM.length content) 0 content
    -- Match the growable owner's final header access before freezing.
    (logicalSize, finalContent) <- readIORef header
    U.unsafeFreeze (UM.unsafeTake logicalSize finalContent)
...
directGrowableUnboxedGrowthKernel input =
  unsafePerformIO do
    initial <- UM.unsafeNew 0
    header <- newIORef (0, initial)
    directGrowthWorker input 0 header
    (_, capacityContent) <- readIORef header
    (logicalSize, content) <- readIORef header
    frozen <- U.unsafeFreeze (UM.unsafeTake logicalSize content)
    NonLinear.pure (frozen, UM.length capacityContent)

directGrowthWorker ::
  U.Vector Int ->
  Int ->
  IORef (Int, UM.IOVector Int) ->
  IO ()
...
directGrowthWorker input !index header
  ...
  | otherwise = do
      ...
      writeIORef header (logicalSize + 1, grown)
...
owningBoxedFftKernel ::
  ...
  B.Vector (Complex Double)
{-# NOINLINE owningBoxedFftKernel #-}
owningBoxedFftKernel input =
  unur $ linearly \linear -> DataFlow.do
    (ownerLinear, runLinear) <- dup linear
    runBO runLinear Control.do
      (vector, lend) <-
        borrowM (OwningVector.fromVector input ownerLinear)
      Control.void $
        OwningDC.sequentialDivideAndConquer
          (OwningDC.fftDC' 128)
          vector
      pureAfter (OwningVector.toVector (reclaim lend))
    ...
    <> [ env
           ...
           \ ~(boxedInput, unboxedInput) ->
             bgroup
               ...
               [ bench "owning/boxed" $
                   nf owningBoxedFftKernel boxedInput
               , bench "unrestricted/boxed" $
