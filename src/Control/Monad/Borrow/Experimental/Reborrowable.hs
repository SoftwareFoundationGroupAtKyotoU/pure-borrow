...
module Control.Monad.Borrow.Experimental.Reborrowable (
  Reborrowable (..),
) where
...
'Control.Monad.Borrow.Unsafe.reviveAlias' for a scalar borrow,
'Control.Monad.Borrow.Experimental.Borrows.reviveAliases' for a bundle.
...
"Control.Monad.Borrow.Internal" for the full argument, and treat it as
...
class (bor ~ WithLifetime bor (LifetimeOf bor)) => Reborrowable bor where
  ...
  locally' ::
    forall a α' r w.
    bor a %1 ->
    (forall β. WithLifetime bor (β /\ LifetimeOf bor) a %1 -> BO' w (β /\ α') (After β r)) %1 ->
    BO' w α' (r, bor a)
  ...
  @'Control.Monad.Borrow.Experimental.Borrows.Muts'@ override it.
  -}
  locally ::
    forall a α' r w.
    bor a %1 ->
    (forall β. WithLifetime bor (β /\ LifetimeOf bor) a %1 -> BO' w (β /\ α') r) %1 ->
    BO' w α' (r, bor a)
