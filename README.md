...
After you import the image, you can run the following for a quick check, which is expected to finish within a second or so:
...
It also generates `qsort.png`, which corresponds to Fig. 13 if the environment has `gnuplot` in `PATH`. In particular, the Docker-based approach should generate `qsort.png`. The plot is generated by the script `scripts/genplot.gnuplot` in the source tarball (`pure-borrow-src.tar.gz`).
...
The generated CSV (or its plot visualization) is expected to support Claim #3 on performance if the benchmarking environment is appropriate (e.g., has sufficiently many cores). Please note that the exact benchmarking results may vary significantly depending on the environment.
...
- Re-exports from `Control.Monad.Borrow.Pure.Lifetime`.
  + Fig. 7a: `Lifetime` type, lifetime inclusion `<=`, `Static` lifetime, and lifetime intersection `(/\)`.
...
- `Control.Concurrent.DivideConquer.Linear` provides a general divide-and-conquer concurrency API based on Pure Borrow. It is used to implement the "work-stealing" version of quicksort.
  + Internally, the API carefully uses unsafe data types (such as OnceChan or Queue), which can break the purity if misused. The API aims to safely encapsulate these data types.
...
Then we apply `app/convert-qsort-bench-csv.hs` to generate the final CSV so that it can be fed into pgfplots. Alternatively, you can use the gnuplot script `scripts/genplot.gnuplot` to render a plot in your environment.

We also developed the `artifact-runner` app (`app/artifact-runner.hs`) to automate the two-step procedure above and enable parameterized benchmarks, where one can customize parameters such as the number of cores or plotting points (unlike `convert-qsort-bench-csv`, which hard-codes these parameters).
