...
copyToVector ::
  ...
  BO' w β (Ur (v a), Borrow bk α (Vector v a))
{-# INLINE copyToVector #-}
copyToVector =
...
get ::
  ...
  Int ->
  Borrow bk α (Vector v a) %1 ->
  BO' w β (Ur a, Borrow bk α (Vector v a))
{-# INLINE get #-}
get index vector =
  case size vector of
    (Ur length_, vector)
...
update ::
  ...
  Mut α (Vector v a) %1 ->
  BO' w β (Ur result, Mut α (Vector v a))
{-# INLINE update #-}
