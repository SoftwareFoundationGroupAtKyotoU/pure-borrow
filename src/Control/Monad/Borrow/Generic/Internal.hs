...
import GHC.Exts (Multiplicity (..), RealWorld, State#, runRW#)
...
instance Data.Functor (BO' w α) where
  fmap f (BO' g) = BO' \s -> case g s of
    (# s', a #) -> (# s', f a #)
  {-# INLINE fmap #-}

instance Control.Functor (BO' w α) where
  fmap f (BO' g) = BO' \s -> case g s of
    (# s', a #) -> (# s', f a #)
  {-# INLINE fmap #-}

instance Data.Applicative (BO' w α) where
...
instance Control.Applicative (BO' w α) where
  pure a = BO' \s -> (# s, a #)
  {-# INLINE pure #-}
...
instance Control.Monad (BO' w α) where
...
unsafeCoerceState# = Unsafe.toLinear unsafeCoerce#
...
It lives in `BO' w` rather than being a pure function because a pure barrier depends on nothing the scope produced, so nothing would stop it -- or the reads that consume its result -- from floating above the scope body.
...
Semantically this is `Control.pure` at 'Alias', and it carries no proof obligation of its own: its argument and result types are identical, so it cannot widen a 'Share' into a 'Mut', relabel a 'Lend', or lengthen a lifetime, and the `BO' w` index it returns at is as free as `Control.pure`'s already is.
...
instance (α >= β, a <: b) => BO' w α a <: BO' w β b where
