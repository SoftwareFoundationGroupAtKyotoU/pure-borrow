...
if (!executable || !baselinePattern || !candidatePattern || !outputCsv) {
  console.error(
    "usage: node bench/run-multi-store-scan-paired.mjs EXE BASELINE_PATTERN CANDIDATE_PATTERN OUTPUT.csv [WARMUP_PAIRS] [PAIRS] [STDEV_PERCENT]",
  );
  process.exit(2);
}
...
const temporaryDirectory = fs.mkdtempSync(
  path.join(os.tmpdir(), "pure-borrow-multi-store-"),
);
