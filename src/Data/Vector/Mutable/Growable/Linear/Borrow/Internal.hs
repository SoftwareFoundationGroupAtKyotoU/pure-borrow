      ...
      (Header _ buffer, duplicateRef) ->
        pop (aff duplicateRef) `lseq`
          unsafeSystemIOToBO do
            !value <- MV.unsafeRead buffer index
            NonLinear.pure (Ur $! copy (UnsafeAlias value))
    ...
    case Ref.unsafeReadRef ref of
      (Header _ buffer, duplicateRef) ->
        pop (aff duplicateRef) `lseq`
          unsafeSystemIOToBO do
            !value <- MV.unsafeRead buffer index
            NonLinear.pure (Ur (copy (UnsafeAlias value)), vector)
...
contents do not change. Reallocation copies each initialized capability and
consumes its retired counterpart, hence both ownership constraints.
-}
reserve ::
  (HasCallStack, Copyable a, Consumable a, α >= β) =>
  Int ->
  Mut α (GrowableVector a) %1 ->
  BO β (Mut α (GrowableVector a))
{-# INLINE reserve #-}
...
Reallocation, when required, copies and retires the old initialized prefix;
this is why the element type must be both 'Copyable' and 'Consumable'.
-}
push ::
  (HasCallStack, Copyable a, Consumable a, α >= β) =>
  a %1 ->
...
copyImmutable source offset target = go 0
  where
    !sourceSize = V.length source
    go !index
      | index >= sourceSize = NonLinear.pure ()
      | otherwise = do
          MV.unsafeWrite target (offset + index) (V.unsafeIndex source index)
          go (index + 1)
    ...
    let !oldCapacity = MV.length buffer
     in if requested <= oldCapacity
          then Control.pure buffer
            ...
            grown <- MV.grow buffer (requested - oldCapacity)
            let !() = consumeInitialized logicalSize buffer
