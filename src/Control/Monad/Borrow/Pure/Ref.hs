...
updateRef :: (α >= β) => (a %1 -> BO β (b, a)) %1 -> Mut α (Ref a) %1 -> BO β (b, Mut α (Ref a))
{-# INLINE updateRef #-}
...
modifyRef :: (α >= β) => (a %1 -> a) %1 -> Mut α (Ref a) %1 -> BO β (Mut α (Ref a))
modifyRef f ma = Control.do
  ((), ma) <- updateRef (Control.pure . ((),) . f) ma
  Control.pure ma

swapRef :: (α >= β) => Mut α (Ref a) %1 -> Mut α (Ref a) %1 -> BO β (Mut α (Ref a), Mut α (Ref a))
{-# INLINE swapRef #-}
swapRef ma ma' =
...
readSharedRef :: (α >= β) => Share α (Ref a) %1 -> BO β (Ur (Share α a))
