...
data UnboxedHeader
  = UnboxedHeader !Int !(UM.IOVector Int)

data BoxedHeader
  = BoxedHeader !Int !(MV.IOVector (Int, Int))
...
worklistDirectOpenOnceRootWithSeed seed target =
  unsafePerformIO do
    ...
    logHeader <-
      newUnboxedHeaderFromVector
        (U.replicate worklistNodeCount 0)
    UnboxedHeader _ adjacency <- readIORef adjacencyHeader
    BoxedHeader _ payload <- readIORef payloadHeader
    UnboxedHeader _ queue <- readIORef queueHeader
    UnboxedHeader _ outputLog <- readIORef logHeader
    finalTraversal <-
      runOpenOnce
        (targetVisits target)
        offsets
...
worklistDirectReopenRootWithSeed seed reopenShape growth target =
  unsafePerformIO do
    ...
    UnboxedHeader _ adjacency <- readIORef adjacencyHeader
    BoxedHeader _ payload <- readIORef payloadHeader
    UnboxedHeader queueSize queue <- readIORef queueHeader
    UnboxedHeader logSize outputLog <- readIORef logHeader
    finishDirect
      finalOutcome
      ( case reopenShape of
...
runFlatReopenDirect ::
  ...
  Int ->
  Int ->
  IO (TraversalState, Int, Int, Int, WorklistOutcome)
runFlatReopenDirect
  growth
  target
  offsets
...
writeTraversalState :: UM.IOVector Int -> TraversalState -> IO ()
...
newUnboxedHeaderFromVector :: U.Vector Int -> IO (IORef UnboxedHeader)
newUnboxedHeaderFromVector vector = do
  buffer <- U.thaw vector
  newIORef (UnboxedHeader (U.length vector) buffer)

newBoxedHeaderFromVector :: V.Vector (Int, Int) -> IO (IORef BoxedHeader)
newBoxedHeaderFromVector vector = do
  buffer <- V.thaw vector
  newIORef (BoxedHeader (V.length vector) buffer)

newUnboxedHeader :: Int -> U.Vector Int -> IO (IORef UnboxedHeader)
newUnboxedHeader requested initial = do
  ...
  newIORef (UnboxedHeader (U.length initial) buffer)

appendUnboxed :: IORef UnboxedHeader -> U.Vector Int -> IO Bool
appendUnboxed header values = do
  UnboxedHeader logicalSize buffer <- readIORef header
