...
{- | A copy of "Control.Monad.Borrow.Pure.OrderingSpec.Owners", built with @-fno-state-hack@: owners handed back after a scope, one kernel per path.

Without the state hack, GHC floats code out of a state-threaded lambda, so only a data dependency on the end of the scope keeps an owner read in the scope's 'After' after the scope's writes.
With 'Control.Monad.Borrow.Pure.Lifetime.Token.withEnd' strict in a nullary 'Control.Monad.Borrow.Pure.Lifetime.Token.EndToken', the @srunBO@ kernels of this copy returned @(0, 0)@.
Keep this copy identical to the original.
...
-}
module PureBorrow.NoStateHack.Owners (
  ownerKernels,
  earlyReclaim,
